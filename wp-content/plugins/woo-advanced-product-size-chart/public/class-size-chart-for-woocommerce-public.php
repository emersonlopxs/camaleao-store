...
class Size_Chart_For_Woocommerce_Public {
            ...
            foreach ($chart_table as $chart) {
                ...
                for ($j = 0; $j < count($chart); $j++) {
                    ...
                    if (!empty($chart[$j])) {
                        ...
                        echo ($i == 0) ? "<th>" . esc_html__($chart[$j], $this->plugin_name) . "</th>" : "<td>" . esc_html__('') . "</td>";
                    }
                }
                echo "</tr>";
                $i++;
    ...
    public function size_chart_custom_product_tab($tabs) {
        ...
        if ($chart_position === 'tab') {
            ...
            $tabs['custom_tab'] = array(
                'title' => esc_html__($tab_label, $this->plugin_name),
                'priority' => 50,
    ...
    public function size_chart_custom_product_tab_content() {
        ...
        if ($chart_id) {
            ...
            require_once plugin_dir_path( __FILE__ ) . 'includes/size-chart-contents.php';
        }
    }
    ...
    public function size_chart_popup_button() {
        ...
        if ($chart_position == 'popup') {
            ...
            ?>
            <div class="button-wrapper"><a class="sc_btn" href="#modal" id="chart-button"><?php esc_html_e($popup_label, $this->plugin_name); ?></a></div>
    ...
    public function size_chart_setting_custom_style() {
        ...
        if (isset($post) && !empty($post)) {
            ...
            <?php if ($table_style == 'minimalistic') { ?>
                    ...
                    .button-wrapper #chart-button{color:<?php echo (isset($size_chart_setting['size-chart-title-color']) && $size_chart_setting['size-chart-title-color'] != '') ? esc_attr($size_chart_setting['size-chart-title-color']) : '#007acc'; ?>}
