...
class Size_Chart_For_Woocommerce_Activator {
    ...
    public static function activate($active_plugin) {
        ...
        if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')), true) && !is_plugin_active_for_network('woocommerce/woocommerce.php')) {
            wp_die("<strong>Size Chart for WooCommerce</strong> Plugin requires <strong>WooCommerce</strong> <a href='" . esc_url(get_admin_url(null, 'plugins.php')) . "'>Plugins page</a>.");
        } else {
    ...
    public function size_chart_cloth_template_html_content($template) {
        ...
        if ($template == 'womens-tshirt') {
            $content_html .= '<img class="alignnone size-medium alignright" src="' . esc_url(plugins_url('admin/images/default-chart/women-t-shirt-top.png', dirname(__FILE__)) ). '" alt="womens-tshirt" width="300" height="300" />';
        } elseif ($template == 'tshirt-shirt') {
            $content_html .= '<img class="alignnone size-medium alignright" src="' . esc_url(plugins_url('admin/images/default-chart/mens-tshirts-and-polo-shirts.jpg', dirname(__FILE__))) . '" alt="tshirt-shirt-chart" width="300" height="300" />';
        }
        $content_html .='</div>
			</div>
		</div>';
