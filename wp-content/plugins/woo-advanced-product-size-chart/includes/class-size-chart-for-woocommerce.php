...
class Size_Chart_For_Woocommerce
{
    ...
    private function define_admin_hooks()
    {
		...
		$this->loader->add_action( 'plugin_row_meta', $plugin_admin, 'size_chart_plugin_row_meta',10,2 );
		
        $this->loader->add_action('admin_head', $plugin_admin, 'size_chart_menu_active');
        if (!empty($get_page) && (($get_page == 'size_chart_setting_page') || ($get_page == 'size_chart_get_started_page') || ($get_page == 'size-chart-information'))) {
            $this->loader->add_filter('admin_footer_text', $plugin_admin, 'size_chart_admin_footer_review');
        }
        if (isset($get_post_type) && !empty($get_post_type) && (($get_post_type == 'size-chart'))) {
            $this->loader->add_filter('admin_footer_text', $plugin_admin, 'size_chart_admin_footer_review');
        }
    }
    ...
    public function plugin_action_links($actions, $plugin_file, $plugin_data, $context)
    {
        $custom_actions = array(
            'configure' => sprintf('<a href="%s">%s</a>', esc_url(admin_url('edit.php?post_type=size-chart&page=size_chart_setting_page')), esc_html__('Settings', $this->plugin_name))
        );
